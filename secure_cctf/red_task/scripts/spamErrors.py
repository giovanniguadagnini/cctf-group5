...
def main():
    if len(sys.argv) != 2:
        print("Usage: spamErrors waitTime | waitTime in seconds, for example 0.05 is 50 milliseconds")
        exit(-1)
    while (1):
        #create a random username + password combo, try to register them (wont work), sleep time is adjustable
